...
def test_reboot_succeeds_with_failed_checks_if_whitelisted(run_cli, forward_port, consul_cluster, reboot_task, mock_subprocess_run, mocker):
...
def test_rebooting_fails_with_failing_checks(run_cli, forward_port, consul_cluster, reboot_task, mock_subprocess_run, mocker):
    consul_cluster[0].agent.service.register("A", tags=["rebootmgr"])
    consul_cluster[1].agent.service.register("A", tags=["rebootmgr"], check=Check.ttl("1ms")) # Failing
    time.sleep(0.01)
