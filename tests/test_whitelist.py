...
def test_reboot_success_whitelisted_checks(run_cli, forward_port, consul_service, consul_cluster, consul_kv, reboot_task, mock_subprocess_run, mocker):
...
def test_reboot_fail_checks(run_cli, forward_port, consul_cluster, consul_service, consul_kv, reboot_task, mock_subprocess_run, mocker):
    consul_service.register(consul_cluster[0], "A", tags=["rebootmgr"])
    consul_service.register(consul_cluster[1], "A", tags=["rebootmgr"], check=Check.ttl("1ms")) # Failing
    time.sleep(0.01)
