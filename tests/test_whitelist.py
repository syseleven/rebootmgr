...
def test_reboot_success_whitelisted_checks(run_cli, forward_port, consul_service, consul1, consul2, consul_kv, reboot_task, mocked_run, mocker):
...
def test_reboot_fail_checks(run_cli, forward_port, consul1, consul2, consul_service, consul_kv, reboot_task, mocked_run, mocker):
    consul_service.register(consul1, "A", tags=["rebootmgr"])
    consul_service.register(consul2, "A", tags=["rebootmgr"], check=Check.ttl("1ms")) # Failing
    time.sleep(0.01)
