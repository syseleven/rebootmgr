...
def test_get_firing_critical_alerts(mock_prometheus_response):
    ...
    with patch('requests.get') as mock_get:
        ...
        alerts = get_firing_critical_alerts(hostname, "https://prometheus:9090/api/v1/alerts")
...
def test_get_firing_critical_alerts_request_exception():
    ...
    with patch('requests.get') as mock_get:
        ...
        mock_get.side_effect = requests.exceptions.RequestException("Test request exception")
...
def test_wait_for_critical_alerts_with_alerts(mocker, consul_cluster):
    ...
    mocker.patch('rebootmgr.main.get_firing_critical_alerts', return_value=[{"state": "firing"}])
    ...
    try:
        with pytest.raises(TimeoutError):
            wait_for_critical_alerts(consul_cluster[0], None, hostname, group_key, prometheus_server_domain)
    finally:
...
def test_post_reboot_with_prometheus(mocker, run_cli, consul_cluster, forward_consul_port, default_config, reboot_in_progress, reboot_task):
...
def test_post_reboot_without_prometheus(mocker, run_cli, consul_cluster, forward_consul_port, default_config, reboot_in_progress, reboot_task):
...
def test_post_reboot_with_timeout(mocker, run_cli, consul_cluster, forward_consul_port, default_config, reboot_in_progress, reboot_task):
    ...
    mocker.patch('rebootmgr.main.wait_for_critical_alerts', side_effect=TimeoutError("Test timeout"))
    reboot_task("post_boot", "50_another_task.sh")
