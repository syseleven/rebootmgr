...
from rebootmgr.main import EXIT_CONSUL_CHECKS_FAILED, EXIT_DID_NOT_REALLY_REBOOT
...
def test_post_reboot_wait_until_healthy(
    ...
    consul_cluster[0].agent.service.register("A", tags=["rebootmgr"])
    consul_cluster[1].agent.service.register(
        "A", tags=["rebootmgr"], check=Check.ttl("1000s")
    )
...
def test_post_reboot_phase_succeeds_with_tasks(
    run_cli, forward_consul_port, default_config, reboot_in_progress, reboot_task
):
...
def test_post_reboot_phase_fails_with_uptime(
    ...
    mocker.patch(
        "rebootmgr.main.open", new=mock_open(read_data="99999999.9 99999999.9")
    )
    ...
    assert (
        "We are in post reboot state but uptime is higher then 2 hours."
        in result.output
    )
